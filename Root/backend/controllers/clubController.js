...
async function getMyClubs(req, res, next) {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT 
          c.MaCLB, c.TenCLB, c.Logo, c.LinhVuc, 
          tv.VaiTroCLB, tv.NgayThamGia, tv.DiemDongGop, tv.TrangThai
        FROM THANH_VIEN tv
...
async function joinClub(req, res, next) {
  try {
    ...
    // Generate new MaYC
    const countResult = await pool.request().query("SELECT COUNT(*) as count FROM YEU_CAU_THAM_GIA_CLB");
    ...
    await pool.request()
      ...
      .input('lyDo', sql.NVarChar, lyDoThamGia || '')
      .query(`
    ...
    return res.status(201).json({
      success: true,
      message: 'Gửi yêu cầu tham gia thành công, vui lòng chờ Ban chủ nhiệm duyệt'
    });
  } catch (error) {
    next(error);
  }
