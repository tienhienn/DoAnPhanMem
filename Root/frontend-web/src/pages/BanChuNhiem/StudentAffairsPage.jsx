...
// ============================================
const FinalApprovalModal = ({ isOpen, event, onClose, onApprove, onReject, loading }) => {
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[95vh] overflow-y-auto border border-slate-100">
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 via-cyan-600 to-teal-600 px-8 py-6 flex items-center justify-between rounded-t-3xl z-10 text-white">
          <div>
            ...
            <p className="text-blue-100 text-sm mt-1">Phê duyệt cấp Phòng CTSV</p>
          </div>
        ...
        <div className="p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
              <p className="text-xs font-semibold text-slate-500 uppercase mb-1">CLB tổ chức</p>
              <p className="text-sm font-semibold text-teal-700">{event.TenCLB || event.MaCLB}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
              <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Thời gian</p>
              <p className="text-sm font-semibold text-slate-900">{formatDate(event.ThoiGianBatDau)}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
          ...
          {event.MoTa && (
            <div>
              ...
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{event.MoTa}</p>
              </div>
            </div>
          )}
          ...
          {event.TrangThai === "pending_student_affairs" && (
            <div>
              <label className="block text-sm font-bold text-slate-700 uppercase tracking-wide mb-3">
                <FiMessageSquare className="inline w-4 h-4 mr-2" />Ý kiến chỉ đạo / Lý do từ chối
              </label>
              <textarea
                value={opinion}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-slate-100 flex flex-col">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6 flex items-center justify-between rounded-t-3xl z-10 text-white flex-shrink-0">
          <div>
            <h2 className="text-2xl font-bold">Thẩm Định Quyết Định Thành Lập CLB</h2>
            <p className="text-blue-100 text-sm mt-1">Hồ sơ: {reg.TenCLB} (Đơn vị quản lý: {reg.tenDVQL})</p>
          </div>
          <button onClick={onClose} disabled={loading} className="p-2 rounded-lg hover:bg-white/20 transition-colors">
            <FiX className="w-6 h-6" />
          </button>
        ...
        <div className="p-8 space-y-8 overflow-y-auto flex-1">
          ...
          <div className="space-y-4">
            <h3 className="text-base font-bold text-slate-800 border-b pb-2 flex items-center gap-2"><FiFileText className="text-blue-600" /> 1. Tờ trình thành lập (BM-CLB-01)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border">
              <div>
                <p className="text-xs font-bold text-slate-500 uppercase">Tên Câu lạc bộ dự kiến</p>
                <p className="text-sm font-bold text-slate-800 mt-1">{reg.TenCLB}</p>
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                <p className="text-xs font-bold text-slate-500 uppercase">Tính cấp thiết</p>
                <p className="text-sm text-slate-700 leading-relaxed mt-1 whitespace-pre-wrap">{step1.tinhCapThiet || reg.MoTa}</p>
              </div>
            </div>
          </div>
          ...
          {/* Điều lệ */}
          <div className="space-y-4">
            <h3 className="text-base font-bold text-slate-800 border-b pb-2 flex items-center gap-2"><FiBookOpen className="text-blue-600" /> 2. Điều lệ hoạt động CLB (BM-CLB-02)</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-2xl border">
              ...
              <div className="md:col-span-3">
                <p className="text-xs font-bold text-slate-500 uppercase">Giới thiệu chung</p>
              ...
              <div className="md:col-span-3">
                <p className="text-xs font-bold text-slate-500 uppercase">Nguyên tắc & Quyền lợi thành viên</p>
          ...
          {/* BCN lâm thời */}
          <div className="space-y-4">
            <h3 className="text-base font-bold text-slate-800 border-b pb-2 flex items-center gap-2"><FiUsers className="text-blue-600" /> 3. Danh sách Ban chủ nhiệm lâm thời (BM-CLB-03)</h3>
          ...
          {/* Kế hoạch */}
          <div className="space-y-4">
            <h3 className="text-base font-bold text-slate-800 border-b pb-2 flex items-center gap-2"><FiCalendar className="text-blue-600" /> 4. Kế hoạch hoạt động & Kinh phí (BM-CLB-05)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border">
              <div>
              ...
              <div className="md:col-span-2">
                <p className="text-xs font-bold text-slate-500 uppercase">Nội dung chi tiết</p>
                <p className="text-sm text-slate-700 mt-1 whitespace-pre-wrap">{step4.noiDungHoatDong || "—"}</p>
              </div>
              <div className="md:col-span-2">
                <p className="text-xs font-bold text-slate-500 uppercase">Kinh phí dự toán</p>
...
export default function StudentAffairsPage() {
  ...
  // ── Stats (Events) ─────────────────────────────────────────────
  const eventPendingCount  = events.filter((e) => e.TrangThai === "pending_student_affairs" && e.KhoaDuyet && !e.PhongCTSVDuyet).length;
  const eventApprovedCount = events.filter((e) => (e.TrangThai === "approved" || e.TrangThai === "sap_dien_ra" || e.TrangThai === "dang_dien_ra") && e.KhoaDuyet && e.PhongCTSVDuyet).length;
  ...
  const filteredEvents = events.filter((e) => {
    ...
    if (activeFilter === "approved") {
      return (e.TrangThai === "approved" || e.TrangThai === "sap_dien_ra" || e.TrangThai === "dang_dien_ra") && e.KhoaDuyet && e.PhongCTSVDuyet;
    }
    return e.TrangThai === "rejected";
  });

  // ── Stats (Clubs) ──────────────────────────────────────────────
  const clubPendingCount  = registrations.filter((r) => r.TrangThai === "pending_student_affairs").length;
  const clubApprovedCount = registrations.filter((r) => r.TrangThai === "approved").length;
  ...
  const handleEventApprove = async (maSK, opinion) => {
    try {
      setLoading(true);
      await apiClient.patch(`/api/admin/events/${maSK}/review`, { status: "approved", feedback: opinion });
  ...
  const handleEventReject = async (maSK, opinion) => {
    try {
      setLoading(true);
      await apiClient.patch(`/api/admin/events/${maSK}/review`, { status: "rejected", feedback: opinion });
  ...
  const handleRegApprove = async (regId, opinion) => {
    try {
      setLoading(true);
      await apiClient.patch(`/api/clubs/admin/registrations/${regId}/review`, { status: "approved", feedback: opinion });
      alert("Quyết định thành lập câu lạc bộ thành công! CLB đã được thêm vào danh sách hoạt động.");
  ...
  const handleRegReject = async (regId, opinion) => {
    try {
      setLoading(true);
      await apiClient.patch(`/api/clubs/admin/registrations/${regId}/review`, { status: "rejected", feedback: opinion });
  ...
  const handleExportReport = () => {
    alert("Đang trích xuất báo cáo. Chức năng xuất Excel sẽ tự động tải xuống.");
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50">
      <div className="p-8">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-slate-800">Quản lý Cấp phép cấp Trường</h1>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            {activeTab === "events" ? (
              ...
              <>
                ...
                {loading ? (
                  <div className="px-6 py-12 text-center text-slate-500">Đang tải...</div>
                  ...
                  <div className="divide-y divide-slate-200">
                    {filteredEvents.map((event) => (
                      ...
                      >
                        ...
                        <div className="text-teal-700 font-medium truncate">{event.TenCLB || event.MaCLB}</div>
                        <div className="text-slate-600">{formatDate(event.ThoiGianBatDau)}</div>
                        <div className="text-slate-600 truncate">{event.DiaDiem}</div>
                        <div className="text-slate-600">{event.SoNguoiToiDa} slot</div>
                        <div>
                  ...
                  <div className="divide-y divide-slate-200">
                    {filteredRegistrations.map((reg) => (
                      ...
                      >
                        ...
                        <div className="text-slate-600 truncate">{reg.tenDVQL}</div>
                        <div className="text-slate-600">{new Date(reg.NgayTao).toLocaleDateString("vi-VN")}</div>
                        <div>
                          <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold ${
