...
export default function ClubDetailPage() {
  ...
  const handleJoin = async (e) => {
    e.preventDefault();
    try {
      const response = await apiClient.post(`/api/clubs/${id}/join`, { lyDoThamGia: joinReason });
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
      <Link to="/clubs" className="inline-flex items-center text-gray-500 hover:text-indigo-600 transition-colors font-medium text-sm mb-2">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-end -mt-16 md:-mt-20 relative z-10 mb-8">
            <div className="w-32 h-32 md:w-40 md:h-40 bg-white rounded-3xl p-1.5 shadow-md">
              <div className="w-full h-full bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 font-bold text-5xl">
                {club.Logo ? <img src={club.Logo} alt="" className="w-full h-full object-cover rounded-2xl" /> : club.TenCLB.charAt(0)}
              </div>
            </div>
            ...
            <div className="flex-1 space-y-2 mb-2">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm mt-2">
                ...
                <span className="flex items-center gap-1 text-gray-500 font-medium bg-gray-50 px-3 py-1 rounded-lg">
                  <Users className="w-4 h-4" /> {club.SoThanhVienHienTai}/{club.SoThanhVienToiDa} Thành viên
                </span>
              </div>
            </div>
              ...
              <section>
                ...
                <div className="prose prose-indigo text-gray-600 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                  {club.MoTa ? (
                    ...
                    <p className="italic text-gray-400">Chưa có thông tin giới thiệu.</p>
                  )}
                </div>
              </section>
              ...
              <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-6">
                ...
                <ul className="space-y-4">
                  <li className="flex items-start gap-3 text-sm">
                    ...
                    <div>
                      <p className="text-gray-500 font-medium mb-0.5">Ngày thành lập</p>
                      <p className="text-gray-800 font-semibold">{new Date(club.NgayThanhLap).toLocaleDateString('vi-VN')}</p>
                    </div>
                  </li>
                  <li className="flex items-start gap-3 text-sm">
                    <Flag className="w-5 h-5 text-gray-400 shrink-0" />
                    <div>
                      <p className="text-gray-500 font-medium mb-0.5">Trạng thái</p>
                      <p className="text-green-600 font-semibold">{club.TrangThai}</p>
                    </div>
                  </li>
                  <li className="flex items-start gap-3 text-sm">
                    <Users className="w-5 h-5 text-gray-400 shrink-0" />
                    <div>
                      <p className="text-gray-500 font-medium mb-0.5">Chủ nhiệm</p>
                      <p className="text-gray-800 font-semibold">{club.TenChuNhiem || 'Chưa cập nhật'}</p>
                    </div>
                  </li>
                </ul>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-gray-100 bg-gray-50">
              <h2 className="text-xl font-bold text-gray-800">Đăng ký tham gia</h2>
              <p className="text-sm text-gray-500 mt-1">Gửi yêu cầu tới {club.TenCLB}</p>
            </div>
            <form onSubmit={handleJoin} className="p-6">
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-2">Lý do tham gia <span className="text-red-500">*</span></label>
                <textarea 
                  required
                  rows={4}
                  value={joinReason}
                ...
                ></textarea>
                <p className="text-xs text-gray-500 mt-2">Ban chủ nhiệm sẽ xem xét dựa trên lý do của bạn.</p>
              </div>
