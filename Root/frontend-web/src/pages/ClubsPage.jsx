...
import { Users, Calendar, Plus, Search, X, Check, FileText, Info, HelpCircle } from 'lucide-react';

const STATUS_CONFIG = {
  pending_faculty: { label: 'Chờ Khoa duyệt', bg: 'bg-amber-50 border-amber-200 text-amber-700' },
  pending_student_affairs: { label: 'Chờ CTSV duyệt', bg: 'bg-blue-50 border-blue-200 text-blue-700' },
  approved: { label: 'Đã phê duyệt', bg: 'bg-emerald-50 border-emerald-200 text-emerald-700' },
  rejected: { label: 'Bị từ chối', bg: 'bg-rose-50 border-rose-200 text-rose-700' }
};
...
export default function ClubsPage() {
    ...
    try {
      ...
      if (unitsRes.data.success) {
        ...
        if (unitsRes.data.data.length > 0) {
          setFormData(prev => ({ ...prev, maDVQL: unitsRes.data.data[0].maDVQL }));
        }
      }
    } catch (err) {
  ...
  const handleAddBcnMember = () => {
    if (!newBcn.hoTen || !newBcn.sdt || !newBcn.email) {
      alert('Vui lòng điền đầy đủ Họ tên, SĐT và Email của thành viên BCN lâm thời.');
      return;
    }
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8">
      ...
      {myRegistrations.length > 0 && !searchTerm && (
        <section className="space-y-4">
          <h2 className="text-lg font-bold text-gray-800 px-2 flex items-center gap-2">
            <span className="w-2 h-6 bg-amber-500 rounded-full"></span> Yêu cầu mở CLB của bạn
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {myRegistrations.map(reg => {
              const cfg = STATUS_CONFIG[reg.TrangThai] || STATUS_CONFIG.pending_faculty;
              return (
                <div key={reg.MaDKMo} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col justify-between space-y-4">
                  <div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-gray-400">Mã đơn: {reg.MaDKMo}</span>
                      <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${cfg.bg}`}>{cfg.label}</span>
                    </div>
                    <h3 className="font-bold text-gray-800 text-lg mt-2">{reg.TenCLB}</h3>
                    <p className="text-sm text-indigo-600 font-medium mt-1">Lĩnh vực: {reg.LinhVuc} | Quản lý: {reg.tenDVQL}</p>
                    <p className="text-xs text-gray-500 mt-2">Ngày nộp: {new Date(reg.NgayTao).toLocaleDateString('vi-VN')}</p>
                    ...
                    {reg.LyDoTuChoi && (
                      <div className="bg-rose-50 border border-rose-100 rounded-xl p-3 mt-3">
                        ...
                        <p className="text-sm text-rose-700 mt-0.5">{reg.LyDoTuChoi}</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      {isOpenRegister && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-md" onClick={() => !submitLoading && setIsOpenRegister(false)}></div>
          <div className="relative bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-gray-100 flex flex-col">
            ...
            <div className="sticky top-0 bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-4 flex items-center justify-between rounded-t-3xl z-10 text-white">
              <div>
                ...
                <p className="text-xs text-indigo-100 mt-0.5">Số hóa hồ sơ thành lập CLB chuẩn</p>
              </div>
              <button onClick={() => !submitLoading && setIsOpenRegister(false)} className="p-2 hover:bg-white/10 rounded-lg transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              {[
                ...
                <div key={s.number} className="flex items-center space-x-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center border font-bold text-xs ${
                    step === s.number ? 'bg-indigo-600 border-indigo-600 text-white shadow-sm' :
                    step > s.number ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-gray-200 text-gray-400'
                  }`}>
                    {step > s.number ? <Check className="w-4 h-4" /> : s.number}
                  </div>
                  <span className={step === s.number ? 'text-indigo-600' : 'text-gray-400'}>{s.label}</span>
                </div>
              ))}
            </div>
            ...
            <div className="p-6 flex-1 space-y-6">
              ...
              {step === 1 && (
                <div className="space-y-4">
                  <h4 className="font-bold text-gray-800 text-sm border-b pb-2 flex items-center gap-2"><FileText className="w-4 h-4 text-indigo-600" /> Bảng 1: Tờ trình thành lập (BM-CLB-01)</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tên Câu lạc bộ dự kiến *</label>
                      <input 
                        type="text" 
                        name="tenCLB" 
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Lĩnh vực hoạt động *</label>
                      <select 
                        name="linhVuc" 
                      ...
                      >
                        <option value="Học thuật">Học thuật / Nghiên cứu</option>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Đơn vị quản lý phê duyệt (Khoa chủ quản/Phòng CTSV) *</label>
                    <select 
                      name="maDVQL" 
                    ...
                    >
                      {units.map(u => (
                        <option key={u.maDVQL} value={u.maDVQL}>{u.tenDVQL}</option>
                      ))}
                    </select>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tính cấp thiết về việc thành lập CLB *</label>
                    <textarea 
                      name="tinhCapThiet" 
              ...
              {step === 2 && (
                <div className="space-y-4">
                  <h4 className="font-bold text-gray-800 text-sm border-b pb-2 flex items-center gap-2"><Info className="w-4 h-4 text-indigo-600" /> Bảng 2: Điều lệ hoạt động CLB (BM-CLB-02)</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tên tiếng Anh</label>
                      <input 
                        type="text" 
                        name="tenTiengAnh" 
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tên viết tắt</label>
                      <input 
                        type="text" 
                        name="tenVietTat" 
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Slogan của CLB</label>
                      <input 
                        type="text" 
                        name="slogan" 
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Giới thiệu khái quát về CLB *</label>
                    <textarea 
                      name="gioiThieu" 
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tôn chỉ, mục đích *</label>
                    <textarea 
                      name="tonChiMucDich" 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Lĩnh vực và Phạm vi hoạt động *</label>
                      <textarea 
                        name="phamViHoatDong" 
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Quyền lợi và Trách nhiệm thành viên *</label>
                      <textarea 
              ...
              {step === 3 && (
                <div className="space-y-4">
                  <h4 className="font-bold text-gray-800 text-sm border-b pb-2 flex items-center gap-2"><Users className="w-4 h-4 text-indigo-600" /> Bảng 3: Nhân sự & BCN lâm thời (BM-CLB-03 & 04)</h4>
                  
                  {/* BCN List Table */}
                  <div className="border border-gray-100 rounded-2xl overflow-hidden bg-gray-50 p-4">
                    <span className="text-xs font-bold text-gray-500 uppercase block mb-3">Thành viên BCN lâm thời hiện có ({bcnList.length})</span>
                    {bcnList.length === 0 ? (
                      <p className="text-sm text-gray-400 text-center py-4">Chưa có thành viên nào.</p>
                    ) : (
                      <div className="space-y-2">
                        {bcnList.map(member => (
                          <div key={member.id} className="bg-white p-3 rounded-xl border border-gray-100 flex items-center justify-between text-sm shadow-sm">
                            <div>
                              <p className="font-bold text-gray-800">{member.hoTen} <span className="text-xs font-normal text-gray-400">({member.email})</span></p>
                              <p className="text-xs text-indigo-600 font-medium">Chức vụ: {member.chucVu} {member.sdt && `| SĐT: ${member.sdt}`}</p>
                            </div>
                            {member.id !== 'leader' && (
                              <button 
                  ...
                  <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-4 space-y-3">
                    <span className="text-xs font-bold text-indigo-600 uppercase block">Thêm nhân sự mới vào danh sách lâm thời</span>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        <input 
                          ...
                          value={newBcn.hoTen}
                          onChange={e => setNewBcn(prev => ({ ...prev, hoTen: e.target.value }))}
                      ...
                      <div>
                        <input 
                          ...
                          value={newBcn.sdt}
                          onChange={e => setNewBcn(prev => ({ ...prev, sdt: e.target.value }))}
                          className="w-full px-3 py-1.5 border rounded-lg text-sm bg-white"
                        />
                      </div>
                      <div>
                        <input 
                          ...
                          value={newBcn.email}
                          onChange={e => setNewBcn(prev => ({ ...prev, email: e.target.value }))}
                          className="w-full px-3 py-1.5 border rounded-lg text-sm bg-white"
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        <select
                          ...
                          onChange={e => setNewBcn(prev => ({ ...prev, gioiTinh: e.target.value }))}
                          className="w-full px-3 py-1.5 border rounded-lg text-sm bg-white"
                        >
              ...
              {step === 4 && (
                <div className="space-y-4">
                  <h4 className="font-bold text-gray-800 text-sm border-b pb-2 flex items-center gap-2"><HelpCircle className="w-4 h-4 text-indigo-600" /> Bảng 4: Kế hoạch hoạt động trong 12 tháng (BM-CLB-05)</h4>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Mục đích - Yêu cầu hoạt động *</label>
                    <textarea 
                      name="mucDichYeuCau" 
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Nội dung hoạt động chi tiết *</label>
                    <textarea 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Dự kiến kinh phí hoạt động *</label>
                      <textarea 
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tiến độ thực hiện *</label>
                      <textarea 
                        name="tienDo" 
            ...
            <div className="sticky bottom-0 border-t border-gray-100 bg-gray-50 px-6 py-4 flex items-center justify-between rounded-b-3xl">
              ...
              <div className="flex items-center gap-3">
                ...
                {step < 4 ? (
                  <button
                    onClick={() => {
                      if (step === 1 && (!formData.tenCLB || !formData.tinhCapThiet)) {
                        alert('Vui lòng điền đầy đủ Tên CLB và Tính cấp thiết.');
                        return;
                      }
                      if (step === 2 && (!formData.gioiThieu || !formData.tonChiMucDich || !formData.phamViHoatDong || !formData.quyenLoiTrachNhiem)) {
                        alert('Vui lòng điền đầy đủ các thông tin bắt buộc trong Điều lệ.');
                        return;
                      }
                      setStep(prev => prev + 1);
                    }}
                  ...
                  <button
                    ...
                    disabled={submitLoading || !formData.mucDichYeuCau || !formData.noiDungHoatDong || !formData.kinhPhiHoatDong || !formData.tienDo}
